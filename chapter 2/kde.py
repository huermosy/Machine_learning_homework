...
def image_read(filename):
    ...
    for i in range(channels):
        img = cv.imread(filename + "/" + imgs[i])      # 打开对应的图片
        img1 = np.array(cv.imread(filename + "/" + imgs[i]))
...
train_data,rows,cols,dims = image_read('D:\\Learning materials\\machine_homework\\training4')    # 载入对应的训练图片,共20个训练样本
test_data = cv.imread('D:\\Learning materials\\machine_homework\\test3\\image21.jpg')          # 载入对应的测试图片，有一个测试样本
...
for i in range(19):      #  共19个训练样本
    for j in range(rows):
        for k in range(cols):
            for l in range(dims):
                P1[j, k, l] = 1 - pow((test_data[j, k, l] - train_data[j, k, l, i])/h,2)
...
plt.savefig("KDE_result_4.jpg")                     # 保存KDE三维色彩图
plt.show()
...
plt.figure()
plt.imshow(binary)
plt.savefig("Binary_4.jpg")         # 保存为jpg格式的图像
plt.show()
